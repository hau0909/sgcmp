        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-base font-bold text-[#0b1c30] font-headline">
                {dict.admin_service_packages.delete_confirm_title}
              </h3>
              <p className="text-sm text-[#434751] mt-1 font-body">
                {dict.admin_service_packages.delete_confirm_desc.replace("{name}", plan.plan_name)}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            ...
            <p className="text-sm text-amber-700 font-medium font-body">
              {dict.admin_service_packages.delete_warning}
            </p>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-1">
            ...
            >
              ...
              {dict.admin_service_packages.delete_plan}
            </button>
          </div>
        </div>
      </div>
...
function PlanCard({
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] font-bold text-[#434751] uppercase tracking-wider font-mono">
                {dict.admin_service_packages.duration_label.replace("{days}", String(plan.duration_days))}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="px-5 py-4 flex flex-col gap-3 flex-1 font-body">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-bold text-[#434751] uppercase tracking-wider">
            {dict.admin_service_packages.price_label}
          </span>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-semibold text-[#0b1c30]">
            {plan.max_guards !== null ? `${plan.max_guards} ${dict.admin_service_packages.accounts_unit}` : dict.admin_service_packages.unlimited}
          </span>
        </div>
      ...
      <div className="border-t border-[#c3c6d3]/60 px-5 py-3 flex items-center justify-between gap-2 bg-[#eff4ff]/30">
        <button
          ...
          className="h-8 px-2.5 rounded-lg text-xs font-semibold text-red-500 hover:text-red-700 hover:bg-red-50 transition-colors flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
        >
          ...
          {dict.admin_service_packages.delete}
        </button>

        <button
          onClick={() => onEdit(plan)}
          className="h-8 px-4 rounded-lg text-xs font-semibold text-[#434751] bg-white hover:bg-[#dce9ff] border border-[#c3c6d3] transition-colors flex items-center justify-center shadow-sm cursor-pointer"
        >
          <Pencil className="w-3.5 h-3.5 mr-1" />
          {dict.admin_service_packages.edit}
        </button>
      </div>
    </div>
  );
...
export default function ServicePackagesPage() {
  ...
  const fetchPlans = useCallback(async () => {
    try {
      ...
      setError(err?.message || dict.admin_service_packages.error_load);
    } finally {
      setLoading(false);
    }
  }, [dict]);
  ...
  const handleFormSuccess = () => {
    if (dialogMode === "add") {
      showToast(dict.admin_service_packages.toast_add_success, "success");
    } else {
      showToast(dict.admin_service_packages.toast_edit_success, "success");
    }
    fetchPlans();
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (!res.success) {
        throw new Error(res.error || dict.admin_service_packages.toast_delete_error);
      }
      showToast(dict.admin_service_packages.toast_delete_success.replace("{name}", deleteTarget.plan_name), "success");
      setDeleteTarget(null);
      await fetchPlans();
    } catch (err: any) {
      showToast(
        err?.message || dict.admin_service_packages.toast_delete_error,
        "error"
      );
      setDeleteTarget(null);
    } finally {
    ...
    <div className="p-6 flex flex-col gap-6 font-body">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-[#0b1c30] font-headline">
              {dict.admin_service_packages.title}
            </h1>
            <p className="text-xs text-[#434751]">
              {dict.admin_service_packages.subtitle}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="h-9 px-4 rounded-lg text-sm font-semibold text-white bg-[#2c5ead] hover:bg-[#024594] transition-colors flex items-center gap-2 cursor-pointer shadow-sm"
          >
            <Plus className="w-4 h-4" />
            {dict.admin_service_packages.add_plan}
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 gap-3 text-[#434751]">
          ...
          <span className="text-sm font-medium">{dict.admin_service_packages.loading}</span>
        </div>
      ) : plans.length === 0 ? (
        /* Empty state */
        <div className="flex flex-col items-center justify-center py-20 gap-3 bg-white rounded-2xl border border-dashed border-[#c3c6d3]">
          ...
          <p className="text-sm text-[#434751] font-medium">
            {dict.admin_service_packages.no_plans}
          </p>
          <button
            onClick={handleOpenAdd}
          ...
          >
            <Plus className="w-4 h-4" />
            {dict.admin_service_packages.create_first_plan}
          </button>
        </div>
      ) : (
        /* Cards Grid */
      ...
      {!loading && plans.length > 0 && (
        <p className="text-xs text-[#434751]">
          {dict.admin_service_packages.total_plans.replace("{count}", String(plans.length))}
        </p>
      )}
