...
export function VerificationDetail({
  ...
  const fetchVerification = React.useCallback(async () => {
    try {
      ...
      setError(
        err?.message ||
          dict.verification_detail?.error_fetch ||
          "Không thể lấy thông tin khảo sát.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCreateSession = async () => {
    try {
      ...
      setError(
        err?.message ||
          dict.verification_detail?.error_create ||
          "Không thể tạo phiên khảo sát.",
      );
    } finally {
  ...
  const handleUpdate = async (status?: VerificationStatus) => {
    ...
    try {
      ...
      setError(
        err?.message ||
          dict.verification_detail?.error_update ||
          "Lỗi cập nhật khảo sát.",
      );
    } finally {
      setIsSimulating(false);
    }
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      setError(
        err?.message ||
          dict.verification_detail?.error_upload ||
          "Lỗi khi upload ảnh.",
      );
    } finally {
      setIsSimulating(false);
  ...
  const handleDeleteImage = async (indexToDelete: number) => {
    ...
    try {
      ...
      setError(
        err?.message ||
          dict.verification_detail?.error_delete ||
          "Lỗi khi xoá ảnh.",
      );
    } finally {
      setIsSimulating(false);
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 h-64">
        ...
        <p className="text-sm text-gray-500">
          {dict.verification_detail?.loading ||
      ...
      {!verification ? (
        <div className="flex flex-col items-center justify-center p-12 bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-2">
            {dict.verification_detail?.no_session || "Chưa có phiên khảo sát"}
          </h3>
          <p className="text-gray-500 mb-6 text-center max-w-md text-sm">
            {dict.verification_detail?.no_session_desc ||
              "Yêu cầu này chưa được tiến hành khảo sát."}{" "}
            {isCompanyAdmin &&
              (dict.verification_detail?.no_session_admin ||
          ...
          {isCompanyAdmin && (
            ...
            >
              ...
              {dict.verification_detail?.create_btn || "Tạo phiên khảo sát"}
            </button>
          )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-outline-variant/30 pb-4">
            <div>
              <h3 className="text-base font-bold text-on-surface flex items-center gap-2">
                ...
                {!isCompanyAdmin
                  ? dict.verification_detail?.customer_title ||
              ...
              <p className="text-xs text-on-surface-variant mt-0.5 font-medium">
                {!isCompanyAdmin
                  ? dict.verification_detail?.customer_desc ||
                    "Điền thông tin khảo sát và đính kèm hình ảnh thực tế tại mục tiêu"
                  : dict.verification_detail?.admin_desc ||
                    "Ghi nhận tình trạng thực tế trước khi báo giá"}
              </p>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="font-bold text-on-surface-variant uppercase tracking-wider text-[11px]">
                {dict.verification_detail?.status || "Trạng thái:"}
              </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              {!isCompanyAdmin && notes && (
                <div className="bg-red-50 text-red-800 border border-red-200 p-3 rounded-lg text-xs space-y-1 shadow-sm">
                  <div className="font-bold flex items-center gap-1.5">
                    ...
                    {dict.verification_detail?.admin_notes ||
                      "Phản hồi từ Admin"}
                  </div>
              ...
              {isCompanyAdmin ? (
                <div className="bg-blue-50/50 text-on-surface border border-blue-200 p-3 rounded-lg text-xs space-y-2 shadow-sm">
                  <div className="font-bold flex items-center gap-1.5 text-blue-700 uppercase tracking-wider text-[11px]">
                    ...
                    {dict.verification_detail?.description ||
                      "Mô tả hiện trạng"}
                  </div>
                  <div className="ml-5.5 whitespace-pre-wrap leading-relaxed">
                    {description || (
                      <span className="text-on-surface-variant italic">
                        {dict.verification_detail?.description_empty ||
                          "Chưa có mô tả."}
                      </span>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={
                      dict.verification_detail?.description_placeholder ||
                      "Nhập mô tả chi tiết về hiện trạng khu vực khảo sát..."
                    }
              ...
              {isCompanyAdmin && (
                <div>
                  <label className="block text-[11px] font-bold text-on-surface-variant uppercase tracking-wider mb-1.5">
                    {dict.verification_detail?.notes || "Ghi chú"}
                  </label>
                  <textarea
                    value={notes}
                    ...
                    placeholder={
                      dict.verification_detail?.notes_placeholder ||
                      "Nhập ghi chú phản hồi khảo sát nếu cần..."
                    }
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="block text-[11px] font-bold text-on-surface-variant uppercase tracking-wider">
                  {dict.verification_detail?.images || "Hình ảnh khảo sát"} (
                  ...
                  canEditImages ? (
                    ...
                    >
                      ...
                      <p className="text-[10px] font-medium">
                        {isDragActive
                          ? dict.verification_detail?.drop_image ||
                            "Thả ảnh vào đây..."
                          : dict.verification_detail?.upload_image ||
                            "Kéo thả hoặc bấm nút để tải ảnh lên"}
                      </p>
                    </label>
                  ) : (
                    <div className="col-span-full py-8 border border-outline-variant border-dashed text-on-surface-variant bg-surface-bright/50 rounded-lg flex flex-col items-center justify-center pointer-events-none">
                      <FileImage className="w-6 h-6 mb-1.5 opacity-55" />
                      <p className="text-[10px] font-medium">
                        {dict.verification_detail?.no_images ||
                      ...
                      >
                        ...
                        {canEditImages && (
                          <button
                            ...
                            title={
                              dict.verification_detail?.delete_image ||
                              "Xóa ảnh này"
                            }
                          >
                            <X className="w-3 h-3" />
                    ...
                    {canEditImages && (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-medium text-center px-2">
                          {isDragActive
                            ? dict.verification_detail?.drop_here ||
                              "Thả ảnh vào đây"
                            : dict.verification_detail?.add_image || "Thêm ảnh"}
                        </span>
                      </label>
                    )}
                  </>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    if (!notes.trim()) {
                      setError(
                        dict.verification_detail?.admin_notes_empty ||
                          "Vui lòng điền Ghi chú khi từ chối khảo sát.",
                      );
                      return;
                    }
                ...
                >
                  ...
                  {dict.verification_detail?.reject_btn || "Từ chối"}
                </button>
                <button
                ...
                >
                  ...
                  {dict.verification_detail?.approve_btn || "Duyệt khảo sát"}
                </button>
              </div>
            </div>
          )}
          ...
          {!isCompanyAdmin &&
            (verification.status === "rejected" ||
              ...
              <div className="pt-4 border-t border-outline-variant/30 flex items-center justify-end gap-4 flex-wrap">
                ...
                >
                  <Check className="w-3.5 h-3.5" />{" "}
                  {dict.verification_detail?.update_btn ||
      ...
      {confirmAction && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface-container-lowest rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-outline-variant">
            <div
              className={`p-4 border-b ${confirmAction === "approved" ? "bg-emerald-50 border-emerald-100" : "bg-red-50 border-red-100"}`}
            >
              <h3
                className={`text-lg font-bold flex items-center gap-2 ${confirmAction === "approved" ? "text-emerald-700" : "text-red-700"}`}
              >
                ...
                {confirmAction === "approved"
                  ? dict.verification_detail?.confirm_approve_title ||
                    "Xác nhận duyệt khảo sát"
                  : dict.verification_detail?.confirm_reject_title ||
                    "Xác nhận từ chối khảo sát"}
              </h3>
            </div>
            ...
            <div className="p-6 space-y-4 text-sm text-on-surface-variant">
              {confirmAction === "approved" ? (
                <p>
                  {dict.verification_detail?.confirm_approve_desc ||
                    "Bạn có chắc chắn muốn duyệt báo cáo khảo sát này? Sau khi duyệt, khách hàng sẽ nhận được thông báo để xem kết quả."}
                </p>
              ) : (
                <div className="space-y-3">
                  <p>
                    {dict.verification_detail?.confirm_reject_desc ||
                      "Bạn đang từ chối bản khảo sát này. Xin lưu ý các thông tin đã ghi nhận:"}
                  </p>
                  <div className="bg-surface-container p-3 rounded-lg border border-outline-variant/50">
                    <p className="font-semibold text-on-surface mb-1">
                      {dict.verification_detail?.admin_notes_desc ||
                        "Ghi chú từ chối:"}
                    </p>
                    <p className="italic">{notes}</p>
                  </div>
                  <p className="text-red-600 font-medium">
                    {dict.verification_detail?.confirm_reject_warn ||
            ...
            <div className="p-4 border-t border-outline-variant/30 flex items-center justify-end gap-3 bg-surface-container/30">
              ...
              >
                {dict.verification_detail?.cancel || "Hủy bỏ"}
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                {confirmAction === "approved"
                  ? dict.verification_detail?.confirm_approve || "Đồng ý duyệt"
                  : dict.verification_detail?.confirm_reject ||
                    "Xác nhận từ chối"}
              </button>
            </div>
          </div>
